...
# Update the update_ui method to use GameState from GameStateMachine
func update_ui(current_state, current_player_index):
	...
	# Get GameStateMachine service to access GameState
	var game_state_machine = get_service("GameStateMachine")
	if !game_state_machine:
		push_error("BaseUIManager: GameStateMachine service not found")
	...
	# Ensure placement state updates weapon buttons
	if current_state == game_state_machine.GameState.BASE_PLACEMENT or current_state == game_state_machine.GameState.WEAPON_PLACEMENT:
		var placement_state = get_service("PlacementState")
		if placement_state and placement_state.has_method("create_weapon_buttons_for_current_state"):
			placement_state.create_weapon_buttons_for_current_state()
