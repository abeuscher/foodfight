...
func initialize(board: Node2D, types: Node):
	...
	# Create placement preview rectangle if it doesn't exist
...
func place_weapon(grid_pos: Vector2) -> bool:
	...
	game_board.get_node("CellManager").occupy_cells(grid_pos, selected_weapon, current_player_id)
	game_board.get_node("VisualManager").create_weapon_sprite(selected_weapon.id, grid_pos, current_player_id)
